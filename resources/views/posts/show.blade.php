...
<pre style="font-family:verdana"><span class="font-weight-bold"><a href="/profile/{{$comment->user->id}}" class="text-dark" style="text-decoration: none;"><img src="{{$comment->user->profile->profileImage()}}" class="rounded-circle w-100" style="max-width:1.8rem;"> {{$comment->user->name}}</a></span> {{$comment->comment}}@if(auth()->user()->id==$comment->user_id&&auth()->user()->id!=$post->user_id||auth()->user()->name=="admin") • <a class=" text-danger" href="/p/{{$post->id}}/delete/{{$comment->id}}" onclick="this.style.display='none'">Delete</a> @endif</pre>

@endforeach
<span id="commentdive"></span>
<br  id="ss"/>
</div>
@if(auth()->user()->id==$post->user_id && $post->comments->count()>0)
<a href="/p/{{$post->id}}/comments" class="text-secondary font-weight-bold" style="text-decoration: none;">View all <span  id="commentsCount">{{$post->comments->count()}}</span> comments and edit</a>   @elseif(auth()->user()->id==$post->user_id) <span class="text-secondary font-weight-bold" id="commentsCount">No comments</span>    @endif       
              <hr/>
...
@endif
	...
	outline: inherit;" onclick="alert('127.0.0.1:8000/p/{{$post->id}}')" class="pl-2"id="Sharebutton"><svg aria-label="Share Post" class="_8-yf5 " fill="#262626" height="24" viewBox="0 0 48 48" width="24"><path d="M47.8 3.8c-.3-.5-.8-.8-1.3-.8h-45C.9 3.1.3 3.5.1 4S0 5.2.4 5.7l15.9 15.6 5.5 22.6c.1.6.6 1 1.2 1.1h.2c.5 0 1-.3 1.3-.7l23.2-39c.4-.4.4-1 .1-1.5zM5.2 6.1h35.5L18 18.7 5.2 6.1zm18.7 33.6l-4.4-18.4L42.4 8.6 23.9 39.7z"></path></svg></button>
              ...
              <a  class="font-weight-bold text-dark" style="text-decoration: none;" href="/p/{{$post->id}}/likes">@endif<span id="likeCount_{{$post->id}}" class="font-weight-bold">{{$post->likes->count()}} </span><strong> likes</strong></a> <br/>
